...
public class FestTruckDAO {
	...
	public List<FestTruckDTO> list(PageObject2 pgObj) {
		...
		try {
			...
			String sql = "select truckno, country, maindish, mngrname, mngrtel, to_char(applydate,'yyyy-mm-dd')applydate, hit from festtruck order by truckno desc ";
	...
	public FestTruckDTO view(int truckno) {
		...
		try {
			...
			con = DBUtil.getConnection();
			// 3. sql문 작성
			String sql = "select truckno, country, maindish, predppl, applyppl, readyfood, mngrname, mngrtel, to_char(applydate, 'yyyy-mm-dd')applydate, hit from festtruck "
					+ " where truckno = ? "; // 변하는
			// 사용
			// 4. 처리문 객체
			pstmt = con.prepareStatement(sql);
	...
	public void write(FestTruckDTO festTruckDTO) {
		...
		try {
			...
			con = DBUtil.getConnection();
			// 3. sql문 작성
			String sql = "insert into festtruck(truckno, country, maindish, predppl, applyppl, readyfood, mngrname, mngrtel) "
					+ " values(festtruck_seq.nextval, ?, ?, ?, ? ,?, ?, ?) "; // 변하는 데이터 대신 ? 사용
			// 4. 처리문 객체
			pstmt = con.prepareStatement(sql);
	...
	public void increase(int truckno) {
		...
		try {
			...
			con = DBUtil.getConnection();
			// 3. sql문 작성
			String sql = "update festtruck set hit = hit + 1 where truckno = ? "; // 변하는 데이터 대신 ? 사용
			// 4. 처리문 객체
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, truckno); // 첫번째 ?에 no를 int로 셋팅
			// 5. 실행 -> select: executeQuery()
	...
	public void update(FestTruckDTO festTruckDTO) {
		...
		try {
			...
			con = DBUtil.getConnection();
			// 3. sql문 작성
			String sql = "update festtruck set country = ?, maindish = ?, predppl = ?, applyppl=?, readyfood=?, mngrname= ?, mngrtel=? "
	...
	public void delete(int truckno) {
		...
		try {
			...
			con = DBUtil.getConnection();
			// 3. sql문 작성
			String sql = "delete from festtruck " + " where truckno = ?"; // 변하는 데이터 대신 ? 사용
			// 4. 처리문 객체
			pstmt = con.prepareStatement(sql);
